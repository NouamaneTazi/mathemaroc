...
export async function getStaticProps(args: GetStaticPropsContext) {
  ...
  const data = await cms().newsPageQuery({
  ...
  return {
    props: {
      locale,
      news: data.newsCollection?.items,
    },
    revalidate: 86400,
  };
}
...
const NewsPage: NextPage<InferGetStaticPropsType<typeof getStaticProps>> = (
  ...
  return (
    <>
      <NextSeo title={i18n["news-title"][locale] as string} />
      <Container as="section" maxW="6xl" p={[4, 8]}>
        <VStack spacing={4} textAlign="center">
          <Heading>{i18n["news-title"][locale]}</Heading>
          <Text pb={4}>{i18n["news-subtitle"][locale]}</Text>
        </VStack>
      </Container>
      ...
      <Container as="section" maxW="6xl" px={[4, 8]}>
        <VStack spacing={[2, 4]}>
          {news?.map(
            (story) =>
              story && <NewsCard key={story.slug} story={story} withNotes />,
          )}
        </VStack>
      </Container>
