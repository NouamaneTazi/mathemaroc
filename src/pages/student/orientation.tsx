...
export async function getStaticProps(args: GetStaticPropsContext) {
  ...
  const data = await cms().orientationsPageQuery({
  ...
  return {
    props: {
      locale,
      orientations: data.eventCollection?.items,
    },
    revalidate: 86400,
  };
}
...
const OrientationsPage: NextPage<InferGetStaticPropsType<typeof getStaticProps>> = (props) => {
  ...
  return (
    <>
      <NextSeo title={i18n["orientations-title"][locale] as string} />
      <Container as="section" maxW="6xl" p={[4, 8]}>
        <VStack spacing={4} textAlign="center">
          <Heading>{i18n["orientations-title"][locale]}</Heading>
          <Text pb={4}>{i18n["orientations-subtitle"][locale]}</Text>
        </VStack>
      </Container>
