...
export async function getStaticProps(args: GetStaticPropsContext) {
  ...
  const data = await cms().journalsPageQuery({
  ...
  return {
    props: {
      locale,
      journals: data.journalCollection?.items,
    },
    revalidate: 60,
  };
}

const JournalsPage: NextPage<InferGetStaticPropsType<typeof getStaticProps>> = (props) => {
  const { journals, locale } = props;
  ...
  return (
    <>
      <NextSeo title={i18n["journals-title"][locale] as string} />
      <Container as="section" maxW="6xl" p={[4, 8]}>
        <VStack spacing={4} textAlign="center">
          <Heading>{i18n["journals-title"][locale]}</Heading>
          <Text pb={4}>{i18n["journals-subtitle"][locale]}</Text>
        </VStack>
      </Container>

      <Container as="section" maxW="6xl" px={[4, 8]}>
        <VStack spacing={[2, 4]}>
          {journals?.map((journal) => journal && <JournalCard key={journal.filename} journal={journal} />)}
        </VStack>
      </Container>
