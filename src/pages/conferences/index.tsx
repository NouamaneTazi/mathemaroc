...
export async function getStaticProps(args: GetStaticPropsContext) {
  ...
  return {
    props: {
      locale,
      conferences: data.eventCollection?.items,
    },
    revalidate: 60,
  };
}

const ConferencesPage: NextPage<InferGetStaticPropsType<typeof getStaticProps>> = (props) => {
  const { conferences, locale } = props;
  ...
  return (
    <>
      <NextSeo title={i18n["conferences-title"][locale] as string} />
      <Container as="section" maxW="6xl" p={[4, 8]}>
        <VStack spacing={4} textAlign="center">
          <Heading>{i18n["conferences-title"][locale]}</Heading>
          <Text pb={4}>{i18n["conferences-subtitle"][locale]}</Text>
        </VStack>
      </Container>

      <Container as="section" maxW="6xl" px={[4, 8]}>
        <VStack spacing={[2, 4]}>
          {conferences?.map((event) => event && <EventCard key={event.title} event={event} withNotes />)}
        </VStack>
      </Container>
